...
# PSScriptAnalyzer's PSAvoidUsingWriteHost rule rejects the host-only write cmdlet, because its
# output cannot be captured, redirected or suppressed. Write-Information puts the same text on the
# information stream, where a caller can capture it, and -InformationAction Continue keeps it on
# screen by default - which this script needs, since its job is to narrate a release to whoever is
# watching it.
function Write-Status {
	...
	$text = if ($Colour -eq 'Default' -or -not $PSStyle) {
		...
		'{0}{1}{2}' -f $PSStyle.Foreground.$Colour, $Message, $PSStyle.Reset
	}
