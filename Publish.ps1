...
# (NuGet trusted publishing + Docker Hub image).
# Usage: .\Publish.ps1
...
# Get version from Nerdbank.GitVersioning via the project's MSBuild targets (the referenced NuGet
# package), so this does not depend on the global 'nbgv' CLI. The GetBuildVersion target must run
# for the computed version (with Git height) to be populated.
$project = Join-Path $PSScriptRoot 'PanoramicData.Maps/PanoramicData.Maps.csproj'
...
Write-Host "✅ Published tag $version — CI will build and push to NuGet + Docker Hub." -ForegroundColor Green
